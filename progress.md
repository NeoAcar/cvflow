...
- [x] Round 2 critique fixes: (1) §6b paragraph rewritten to be consistent with the H9 contamination callout (146× scaling number removed); (2) §3 main table bold winners annotated with `§` for NULL bootstrap cells, header note added; (3) GMFlow-refine latency **measured** at n=50 (median 652 ms / 1333 MB peak VRAM) — replaces the earlier "~1000 ms" estimate; §5 Pareto rewritten as honest two-batch table; (4) H10 "in-domain over-fitting" mislabel corrected to "motion-regime effect" (both checkpoints are Things-trained, neither saw Sintel); (5) multiplicity-correction caveat added to §11a and §9 — borderline-significant cells flagged; (6) percentile-vs-BCa bootstrap note in §9.
